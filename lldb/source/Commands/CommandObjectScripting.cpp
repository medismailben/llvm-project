...
private:
  ...
  void
  OutputJsonFormat(const llvm::StringMap<std::vector<size_t>> &grouped_by_extension,
    ...
    for (const auto &extension_pair : grouped_by_extension) {
      ...
      for (const std::string &lang : GetLanguagesForExtension(extension_pair.second))
  ...
  void
  OutputTextFormat(const llvm::StringMap<std::vector<size_t>> &grouped_by_extension,
                   CommandReturnObject &result) {
