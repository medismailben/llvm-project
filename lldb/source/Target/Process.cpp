...
void Process::SetPublicState(StateType new_state, bool restarted) {
  ...
  Log *log(GetLog(LLDBLog::State | LLDBLog::Process));
  LLDB_LOGF(log, "Process::SetPublicState (plugin = %s, state = %s, restarted = %i)",
  ...
  if (!StateChangedIsExternallyHijacked()) {
    if (new_state == eStateDetached) {
      LLDB_LOGF(log,
                "Process::SetPublicState (plugin = %s, state = %s) -- unlocking run lock for detach",
      ...
      if ((old_state_is_stopped != new_state_is_stopped)) {
        if (new_state_is_stopped && !restarted) {
          LLDB_LOGF(log, "Process::SetPublicState (plugin = %s, state = %s) -- unlocking run lock",
                    GetPluginName().data(), StateAsCString(new_state));
          m_public_run_lock.SetStopped();
        }
      }
...
Status Process::Resume() {
  Log *log(GetLog(LLDBLog::State | LLDBLog::Process));
  LLDB_LOGF(log, "Process::Resume (plugin = %s) -- locking run lock", GetPluginName().data());
  if (!m_public_run_lock.TrySetRunning()) {
    Status error("Resume request failed - process still running.");
    LLDB_LOGF(log, "Process::Resume (plugin = %s) -- TrySetRunning failed, not resuming.", GetPluginName().data());
    return error;
  }
...
void Process::SetPrivateState(StateType new_state) {
  ...
  if (state_changed) {
    ...
    if (StateIsStoppedState(new_state, false)) {
      ...
      m_memory_cache.Clear();
      LLDB_LOGF(log, "Process::SetPrivateState (plugin = %s, state = %s, stop_id = %u",
                GetPluginName().data(), StateAsCString(new_state), m_mod_id.GetStopID());
    }

    m_private_state_broadcaster.BroadcastEvent(event_sp);
  } else {
    LLDB_LOGF(log,
              "Process::SetPrivateState (plugin = %s, state = %s) state didn't change. Ignoring...",
              GetPluginName().data(), StateAsCString(new_state));
  }
}
