...
#undef SET_CASE_AND_CONTINUE
  ...
  T CallStaticMethod(llvm::StringRef class_name, llvm::StringRef method_name,
    ...
    if (llvm::Error e = expected_return_object.takeError()) {
      ...
      // Create Status with full context including interface type
      error = Status::FromErrorStringWithFormatv(
          "Python exception in {0} method '{1}':\n{2}",
          GetClassName(), method_name, detailed_error);
...
protected:
  ...
  // Optional callback for surfacing errors to users (e.g., CommandReturnObject)
  // If not set, errors are reported via Debugger::ReportError()
