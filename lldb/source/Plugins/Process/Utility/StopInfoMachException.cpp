...
StopInfoSP StopInfoMachException::CreateStopReasonWithMachException(
  ...
  case 6: // EXC_BREAKPOINT
  {
    ...
    if (stopped_by_hitting_breakpoint) {
      ...
      // A fast conditional breakpoint traps on an instruction compiled into its
      // JIT-ed condition, not on a trap lldb installed, and its site is
      // registered under the address it patched instead. So the lookups above
      // find nothing and the stop has to be matched by trap address.
      addr_t injected_trap_addr = LLDB_INVALID_ADDRESS;
      if (!bp_site_sp) {
        const addr_t trap_addr = address ? *address : pc;
        bp_site_sp = process_sp->FindInjectedSiteByTrapAddress(trap_addr);
        if (bp_site_sp)
