...
bool CFBasicHash::UpdateFor(std::unique_ptr<__CFBasicHash<T>> &m_ht) {
  ...
  DataBufferSP buff(new DataBufferHeap(size, 0));
  m_exe_ctx_ref.GetProcessSP()->ReadMemory(addr, buff->GetBytes(), size, error);
  if (error.Fail())
    return false;

  DataExtractorSP data_extractor(
      new DataExtractor(buff, m_byte_order, m_ptr_size));

  m_ht = std::make_unique<__CFBasicHash<T>>();
  size_t copied = data_extractor->CopyData(0, size, m_ht.get());
  ...
  buff.reset(new DataBufferHeap(size, 0));
  m_exe_ctx_ref.GetProcessSP()->ReadMemory(ptr_offset, buff->GetBytes(), size,
                                           error);
  ...
  data_extractor.reset(new DataExtractor(buff, m_byte_order, m_ptr_size));

  copied = data_extractor->CopyData(0, size, m_ht->pointers);
