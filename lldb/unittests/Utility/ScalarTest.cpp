...
bool checkIntegralInequality(T c1, T c2) {
  return (Scalar(llvm::APInt(sizeof(T), c1)) != Scalar(llvm::APInt(sizeof(T), c2)));
}
...
TEST(ScalarTest, Comparison) {
  auto s1 = Scalar(llvm::APInt(sizeof(int), 23));
  auto s2 = Scalar(llvm::APInt(sizeof(int), 46));
  ASSERT_TRUE(s1 < s2);
  ASSERT_TRUE(s1 <= s2);
  ASSERT_TRUE(s2 > s1);
  ASSERT_TRUE(s2 >= s1);
...
TEST(ScalarTest, RightShiftOperator) {
  ...
  int b = 0xFFFFFFFF;
  int c = 4;
  Scalar a_scalar(llvm::APInt(sizeof(int), a));
  Scalar b_scalar(llvm::APInt(sizeof(int), b));
  Scalar c_scalar(llvm::APInt(sizeof(int), c));
  ASSERT_EQ(llvm::APInt(sizeof(int), a >> c), a_scalar >> c_scalar);
  ASSERT_EQ(llvm::APInt(sizeof(int), b >> c), b_scalar >> c_scalar);
}
...
TEST(ScalarTest, GetBytes) {
  ...
  Scalar a_scalar(llvm::APInt(sizeof(int), a));
  Scalar b_scalar(llvm::APInt(sizeof(long long), b));
...
TEST(ScalarTest, CastOperations) {
  ...
  Scalar a_scalar(llvm::APInt(sizeof(long long), a));
  ...
  int a2 = 23;
  Scalar a2_scalar(llvm::APInt(sizeof(int), a2));
...
TEST(ScalarTest, ExtractBitfield) {
  ...
  Scalar s_scalar(llvm::APInt(sizeof(long long), a1));
...
static std::string ScalarGetValue(T value) {
  StreamString stream;
  Scalar(llvm::APInt(sizeof(T), value, std::is_signed<T>::value)).GetValue(&stream, false);
...
TEST(ScalarTest, Promotion) {
  ...
  for (int i = 0; int_types[i] != Scalar::e_void; ++i) {
    for (int j = 0; float_types[j] != Scalar::e_void; ++j) {
      Scalar lhs(llvm::APInt(sizeof(int), 2));
...
TEST(ScalarTest, SetValueFromCString) {
  ...
  EXPECT_THAT_ERROR(
      ...
      Succeeded());
  EXPECT_EQ(Scalar(llvm::APInt(sizeof(unsigned long long), 1234567890123ull)), a);

  EXPECT_THAT_ERROR(
      a.SetValueFromCString("-1234567890123", lldb::eEncodingSint, 8).ToError(),
      Succeeded());
  EXPECT_EQ(Scalar(llvm::APInt(sizeof(long long), -1234567890123ll)), a);
