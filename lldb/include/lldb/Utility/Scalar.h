...
public:
  ...
  Scalar(llvm::APInt v) : m_type(), m_float(static_cast<float>(0)) {
    ...
    m_type = GetBestTypeForBitSize(m_integer.getBitWidth(), m_integer.isSignedIntN(m_integer.getBitWidth()));
  }
  ...
  Scalar(T v) : m_type(), m_float(static_cast<float>(0)) {
    m_integer = llvm::APInt(sizeof(T) * 8, v);
    m_type = GetBestTypeForBitSize(m_integer.getBitWidth(), std::is_signed<T>::value);
  }
  
  template <typename T, std::enable_if_t<std::is_floating_point<T>::value, int> = 0>
  Scalar(T v) : m_type(), m_integer(1, 0, false), m_float(v) {
    m_type = (std::is_same<float, typename std::remove_cv<T>::type>::value) ? e_float : e_double;
  }
  
  // Special constructor for `long double`
  template<typename T,
      std::enable_if_t<std::is_same<T, typename std::remove_cv<long double>::type>::value, int> = 0>
